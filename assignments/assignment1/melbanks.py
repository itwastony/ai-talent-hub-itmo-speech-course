...
class LogMelFilterBanks(nn.Module):
    def __init__(
        ...
        self.f_max_hz = self.samplerate // 2
    ...
    def forward(self, x):
        ...
        """
        stft = self.spectrogram(x)
        power_stft = torch.pow(torch.abs(stft), self.power)
        # batch, n_mels, n_frames
        mel_spec = torch.matmul(power_stft.transpose(1, 2), self.mel_fbanks).transpose(
            1, 2
        )
